...
function! kronos#database#write(data)
  try
    let prev_data = kronos#database#read()
    let next_data = s:assign(prev_data, a:data)
...
function! s:assign(base, override)
  return map(copy(a:base), 's:assign_by_key(a:base, a:override, v:key)')
endfunction

function! s:assign_by_key(base, override, key)
